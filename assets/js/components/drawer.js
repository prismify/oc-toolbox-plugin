...
+function ($) { "use strict";
    ...
    Drawer.prototype.init = function(){
        ...
        /*
         * Do not allow the same drawer to open twice
         */
            ...
            this.$el.request(this.options.handler, {
                ...
                success: function(data, textStatus, jqXHR) {
                    this.success(data, textStatus, jqXHR).done(function(){
                        ...
                        self.triggerEvent('complete.oc.drawer')
                    })
                },
    ...
    Drawer.prototype.initEvents = function(){
        ...
        /*
         * Duplicate the drawer reference on the .control-drawer container
         */
        ...
        this.$modal.on('hide.bs.modal', function(){
            self.triggerEvent('hide.oc.drawer')
            self.isOpen = false
            self.setBackdrop(false)
        })

        this.$modal.on('hidden.bs.modal', function(){
            self.triggerEvent('hidden.oc.drawer')
        ...
        this.$modal.on('shown.bs.modal', function(){
            self.triggerEvent('shown.oc.drawer')
        })

        this.$modal.on('close.oc.drawer', function(){
            self.hide()
            return false
        })
    }
    ...
    Drawer.prototype.createDrawerContainer = function() {
        var modal = $('<div />').prop({
                class: 'control-drawer drawer fade',
                role: 'dialog',
                tabindex: -1
            }),
            modalDialog = $('<div />').addClass('drawer-dialog'),
            modalContent = $('<div />').addClass('drawer-content')
    ...
    Drawer.prototype.setContent = function(contents) {
        this.$content.html(contents)
        this.setLoading(false)
        this.show()

        // Duplicate the drawer object reference on to the first div
        // inside the drawer. Eg: $('#firstDiv').drawer('hide')
    ...
    Drawer.prototype.setBackdrop = function(val) {
        if (val && !this.$backdrop) {
            this.$backdrop = $('<div class="drawer-backdrop fade" />')
    ...
    Drawer.prototype.setLoading = function(val) {
        if (!this.$backdrop)
            return;
    ...
    Drawer.prototype.setShake = function() {
        var self = this

        this.$content.addClass('drawer-shaking')

        setTimeout(function() {
            self.$content.removeClass('drawer-shaking')
        }, 1000)
    }

    Drawer.prototype.hideLoading = function(val) {
        this.setLoading(false)
    ...
    Drawer.prototype.triggerEvent = function(eventName, params) {
        if (!params) {
    ...
    Drawer.prototype.reload = function() {
        this.init()
    }
    ...
    /*
     * Hide the drawer without destroying it,
     * you should call .hide() once finished
     */
    Drawer.prototype.visible = function(val) {
        if (val) {
    ...
    Drawer.prototype.toggle = function() {
        this.triggerEvent('toggle.oc.drawer', [this.$modal])

        this.$modal.modal('toggle')
    }

    /*
     * Lock the drawer from closing
     */
    Drawer.prototype.lock = function(val) {
        this.allowHide = !val
    }
    ...
    $.drawer = function (option) {
        return $('<a />').drawer(option)
    }
    ...
    $(document).on('click.oc.drawer', '[data-control="drawer"]', function(event) {
    ...
    /*
     * Only use the drawer loading indicator if the handlers are an exact match.
     */
    $(document)
        .on('ajaxPromise', '[data-drawer-load-indicator]', function(event, context) {
            if ($(this).data('request') != context.handler) return
            $(this).closest('.control-drawer').removeClass('in').drawer('setLoading', true)
        })
        .on('ajaxFail', '[data-drawer-load-indicator]', function(event, context) {
            if ($(this).data('request') != context.handler) return
            $(this).closest('.control-drawer').addClass('in').drawer('setLoading', false).drawer('setShake')
        })
        .on('ajaxDone', '[data-drawer-load-indicator]', function(event, context) {
            if ($(this).data('request') != context.handler) return
            $(this).closest('.control-drawer').drawer('hideLoading')
        })
